...
const FixedWebcamTryOn: React.FC = () => {
    ...
    const addProductToOverlay = useCallback((product: Product) => {
        ...
        const img = new Image();
        // Without this, drawing the (cross-origin) product image onto the canvas
        // taints it, and Generate Preview's canvas.toDataURL() throws instead of
        // producing a preview. Unsplash sends Access-Control-Allow-Origin: *.
    ...
    const handlePointerDown = useCallback((e: React.PointerEvent<HTMLDivElement>) => {
        // The buttons overlaid on the canvas (Generate Preview, AI Try-On, etc.)
        // sit inside this same hit-test region. Without this guard, pressing one
        // that happens to fall within an overlay item's bounding box captures the
        // pointer on this div instead, so the click event never reaches the button.
