...
const FAILED_LOG_PATH = path.join(__dirname, "..", "revert-failed.log");
const PROGRESS_LOG_PATH = path.join(__dirname, "..", "revert-progress.log");
...
async function revertOne(id: string): Promise<RevertResult> {
  ...
  try {
    ...
    const { error: uploadError } = await supabase.storage
      .from(STORAGE_BUCKET)
      .upload(storagePath, sourceBuffer, {
    ...
    return { id, success: true, byteSize: sourceBuffer.length };
  } catch (err) {
...
async function main(): Promise<void> {
  ...
  console.log(`Found ${total} rows total, ${completed.size} already reverted, ${ids.length} remaining.`);
  ...
  for (let i = 0; i < ids.length; i += CONCURRENCY) {
    ...
    const results = await Promise.all(chunk.map(revertOne));
  ...
  console.log(`Average original image size: ${avgBytes} bytes (~${(avgBytes / 1024).toFixed(1)} KB)`);
  console.log(`Total original bytes uploaded: ${totalBytes} (~${(totalBytes / (1024 * 1024)).toFixed(1)} MB)`);
}
